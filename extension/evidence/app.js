...
function select(id,center=false){if(!multi.has(id))multi.clear();selected=id;if(center&&get(id)){const n=get(id);if(board.sample&&n.route&&n.route!==route()){allRoutes=true;$('#all-routes').checked=true;}render();const p=pos(n);$('#viewport').scrollTo({left:Math.max(0,(p.x-70)*zoom),top:Math.max(0,(p.y-55)*zoom),behavior:'smooth'});}else render();}
...
document.addEventListener('click',e=>{const el=e.target.closest('[data-select],[data-deselect],[data-walk-step],#prev-step,#next-step,[data-node]');if(!el||e.target.closest('a,[data-edit],[data-inspect-card],#edit-heading,[data-connect-handle]'))return;if(el.hasAttribute('data-select'))activateCard(el.dataset.select,true);else if(el.hasAttribute('data-deselect')){selected=null;multi.clear();render()}else if(el.hasAttribute('data-walk-step')){step=Number(el.dataset.walkStep);renderWalk()}else if(el.id==='prev-step'){step--;renderWalk()}else if(el.id==='next-step'){if(step===sequence().length-1){walking=false;render()}else{step++;renderWalk()}}else if(el.dataset.node&&!dragMoved){if((e.shiftKey||e.metaKey||e.ctrlKey)&&author&&!readerMode)toggleMulti(el.dataset.node);else{multi.clear();activateCard(el.dataset.node)}}});
...
$('#viewport').addEventListener('pointerdown',e=>{if(e.button!==0||e.target.closest('a,button'))return;const vp=$('#viewport'),el=e.target.closest('[data-node]'),n=el&&get(el.dataset.node);if(n&&!author)return;
 ...
 const move=ev=>{const dx=ev.clientX-sx,dy=ev.clientY-sy;if(Math.hypot(dx,dy)<4&&!dragMoved)return;dragMoved=true;
  if(group.length){for(const o of origin){o.g.x=Math.max(0,o.x+dx/zoom);o.g.y=Math.max(0,o.y+dy/zoom);const p=pos(o.g);if(o.el){o.el.style.left=p.x+'px';o.el.style.top=p.y+'px'}}renderEdges()}
...
document.addEventListener('keydown',e=>{if(readerMode||walking||tab!=='map'||$('dialog[open]')||e.target.matches('input,textarea,select,[contenteditable]'))return;
 ...
 else if(author&&(e.key==='Delete'||e.key==='Backspace')&&(multi.size||(selected&&e.target.matches('#canvas [data-node]')))){e.preventDefault();removeCards(multi.size?[...multi]:[selected])}});
...
function validateBoard(input){
 ...
 return {version:1,title:input.title.slice(0,500),subtitle:str(input.subtitle)?input.subtitle:'',sample:input.sample===true,answerId:typeof input.answerId==='string'&&nodes.some(n=>n.id===input.answerId&&n.type==='conclusion')?input.answerId:undefined,kind:input.kind==='trade'?'trade':'freelance',profit:input.profit==='above'?'above':'below',nodes,links,steps:[...input.steps]};
}
...
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='z'&&!e.target.matches('input,textarea,select,[contenteditable]')&&!$('dialog[open]')){e.preventDefault();undo(e.shiftKey)}if(e.target.matches('#canvas [data-node]')&&author&&!readerMode&&!walking&&!$('dialog[open]')&&['ArrowLeft','ArrowRight','ArrowUp','ArrowDown'].includes(e.key)){e.preventDefault();const n=get(e.target.dataset.node),amount=e.shiftKey?50:10;for(const m of (multi.has(n.id)?[...multi].map(get).filter(Boolean):[n])){delete m.auto;m.x=Math.max(0,m.x+(e.key==='ArrowLeft'?-amount:e.key==='ArrowRight'?amount:0));m.y=Math.max(0,m.y+(e.key==='ArrowUp'?-amount:e.key==='ArrowDown'?amount:0))}persist();render();$(`#canvas [data-node="${CSS.escape(n.id)}"]`)?.focus()}});
...
const api={get board(){return board},get session(){return session},workspace,get,uid,esc,notify,render,persist,freePosition,select,validateBoard,
 ...
 setAuthor(v){author=!!v;render()},get inboxOpen(){return inboxOpen},setInbox(v){inboxOpen=!!v;render()},get zoom(){return zoom},canvasPoint(cx,cy){const r=$('#canvas').getBoundingClientRect();return {x:Math.max(0,Math.round((cx-r.left)/zoom)),y:Math.max(0,Math.round((cy-r.top)/zoom))}},get tab(){return tab},wordLabel,allowedWords,fitWord,linkRole,supportersOf,WORD_CHOICES,mainConclusion,isInterim,setTab(t){tab=t;render()},openCard,onRender(f){renderHooks.push(f)},commit(record=true){commitBoard(record)},typeLabel,startWalk(){walking=true;step=0;tab='map';render()},currentStep:()=>walking?sequence()[step]?.id:null,sequence:()=>sequence().map(n=>n.id)};
