...
function render(){
 ...
 $('.segmented').hidden=readerMode||tab==='outline';$('#mode-hint').textContent=tab==='outline'&&!readerMode?'Outline · type to edit':readerMode?'Read-only export':author?'Click to select · ✎ or double-click to edit · drag → to connect':'Viewing · switch to Author to edit';
...
document.addEventListener('click',e=>{const el=e.target.closest('[data-select],[data-deselect],[data-walk-step],#prev-step,#next-step,[data-node]');if(!el||e.target.closest('a,[data-edit],[data-inspect-card],#edit-heading,[data-connect-handle]'))return;if(el.hasAttribute('data-select'))activateCard(el.dataset.select,true);else if(el.hasAttribute('data-deselect')){selected=null;render()}else if(el.hasAttribute('data-walk-step')){step=Number(el.dataset.walkStep);renderWalk()}else if(el.id==='prev-step'){step--;renderWalk()}else if(el.id==='next-step'){if(step===sequence().length-1){walking=false;render()}else{step++;renderWalk()}}else if(el.dataset.node&&!dragMoved)activateCard(el.dataset.node);});
