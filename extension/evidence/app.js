...
function render(){for(const id of multi)if(!get(id))multi.delete(id);for(const id of groupSel)if(!groupById(id))groupSel.delete(id);
 ...
 {const ids=board.nodes.map(n=>n.id),g=(joinGroup&&groupById(joinGroup))||(focusGroup&&groupById(focusGroup));if(render.known&&g){const fresh=ids.filter(id=>!render.known.has(id)&&!groupList().some(x=>x.members.includes(id)));if(fresh.length){g.members.push(...fresh);queueMicrotask(persist)}}render.known=new Set(ids)}
...
let placeAt=null,joinGroup=null,focusGroup=null,groupSel=new Set(),panelPinned=false,lastPanelKey='';
...
// Double-click empty canvas: pick what to add, and it lands where you clicked
// (inside a group's outline, it joins that group).
const QUICK_ADD=[['note','Thought','T'],['fact','Fact','F'],['claim','Claim','C'],['conclusion','Conclusion','N'],['gap','Open question','Q'],['evidence','Evidence','E']];
function quickAdd(e){if(readerMode||tab!=='map'||walking)return;if(e.target.closest('[data-node],.group-card,.group-tab,button,a,input,.marquee'))return;
 const p=canvasAt(e.clientX,e.clientY);let inner=null,area=Infinity;for(const el of $$('#canvas .group')){const x=el.offsetLeft,y=el.offsetTop,w=el.offsetWidth,h=el.offsetHeight;if(p.x>=x&&p.x<=x+w&&p.y>=y&&p.y<=y+h&&w*h<area){inner=el.dataset.group;area=w*h}}
 $('#quick-add')?.remove();const g=inner&&groupById(inner);const menu=document.createElement('div');menu.id='quick-add';menu.className='quick-add';menu.setAttribute('role','menu');
 menu.innerHTML=`<p class="quick-add-head">Add here</p>${g?`<p class="quick-add-in">in “${esc((l=>l.length>34?l.slice(0,33)+'…':l)(g.label||'Untitled group'))}”</p>`:''}${QUICK_ADD.map(([t,l,k])=>`<button role="menuitem" data-quick="${t}"><span>${l}</span><kbd>${k}</kbd></button>`).join('')}`;
 document.body.append(menu);const r=menu.getBoundingClientRect();menu.style.left=Math.min(e.clientX,innerWidth-r.width-8)+'px';menu.style.top=Math.min(e.clientY,innerHeight-r.height-8)+'px';
 const close=()=>{menu.remove();removeEventListener('mousedown',outside,true);removeEventListener('keydown',key,true)};
 const pick=type=>{close();author=true;placeAt={x:Math.round(p.x-20),y:Math.round(p.y-16)};joinGroup=inner||null;render();openCard(type)};
 const outside=ev=>{if(!menu.contains(ev.target))close()};
 const key=ev=>{if(ev.key==='Escape'){ev.preventDefault();ev.stopPropagation();close();return}const hit=QUICK_ADD.find(q=>q[2]===ev.key.toUpperCase());if(hit&&!ev.metaKey&&!ev.ctrlKey){ev.preventDefault();ev.stopPropagation();pick(hit[0])}else if(['ArrowDown','ArrowUp'].includes(ev.key)){ev.preventDefault();const bs=[...menu.querySelectorAll('button')],i=bs.indexOf(document.activeElement);bs[(i+(ev.key==='ArrowDown'?1:-1)+bs.length)%bs.length].focus()}};
 menu.addEventListener('click',ev=>{const b=ev.target.closest('[data-quick]');if(b)pick(b.dataset.quick)});
 addEventListener('mousedown',outside,true);addEventListener('keydown',key,true);menu.querySelector('button').focus()}
$('#viewport').addEventListener('dblclick',quickAdd);
$('#editor').addEventListener('close',()=>setTimeout(()=>{placeAt=null;joinGroup=null},0));
