...
const visibleNodes=()=>board.nodes.filter(n=>(allRoutes||active(n))&&!evidenceHidden(n));
...
function select(id,center=false){if(!multi.has(id))multi.clear();const target=get(id);if(target&&evidenceHidden(target)){neighbors(id).filter(x=>x.type!=='evidence').forEach(x=>evidenceView.open.add(x.id));saveEvidenceView();api?.requestLayout?.()}selected=id;if(center&&get(id)){const n=get(id);if(board.sample&&n.route&&n.route!==route()){allRoutes=true;$('#all-routes').checked=true;}render();const p=pos(n);$('#viewport').scrollTo({left:Math.max(0,(p.x-70)*zoom),top:Math.max(0,(p.y-55)*zoom),behavior:'smooth'});}else render();}
...
const api={get board(){return board},get session(){return session},workspace,get,uid,esc,notify,render,persist,freePosition,select,validateBoard,
 ...
 setAuthor(v){author=!!v;render()},get inboxOpen(){return inboxOpen},setInbox(v){inboxOpen=!!v;render()},get zoom(){return zoom},canvasPoint(cx,cy){const r=$('#canvas').getBoundingClientRect();return {x:Math.max(0,Math.round((cx-r.left)/zoom)),y:Math.max(0,Math.round((cy-r.top)/zoom))}},get tab(){return tab},isHidden:id=>{const n=get(id);return !!n&&evidenceHidden(n)},showEvidenceFor,wordLabel,allowedWords,fitWord,linkRole,supportersOf,WORD_CHOICES,mainConclusion,isInterim,setTab(t){tab=t;render()},openCard,onRender(f){renderHooks.push(f)},commit(record=true){commitBoard(record)},typeLabel,startWalk(){walking=true;step=0;tab='map';render()},currentStep:()=>walking?sequence()[step]?.id:null,sequence:()=>sequence().map(n=>n.id)};
