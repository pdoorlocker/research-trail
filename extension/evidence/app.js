...
function mainConclusion(){const pick=get(board.answerId);if(pick?.type==='conclusion'&&active(pick))return pick;const all=board.nodes.filter(n=>n.type==='conclusion'&&active(n));return all.find(n=>!isInterim(n))||all[0]}const renderHooks=[];
...
const ZOOM_MIN=.25,ZOOM_MAX=2,clampZoom=z=>Math.min(ZOOM_MAX,Math.max(ZOOM_MIN,z));
...
function layoutCanvas(at){const vp=document.getElementById('viewport');if(vp.hidden||!vp.clientWidth)return;vpSize={w:vp.clientWidth,h:vp.clientHeight};
 ...
 const c=document.getElementById('canvas');c.style.left=origin.x+'px';c.style.top=origin.y+'px';c.style.transform=`scale(${zoom})`;c.style.setProperty('--gz',Math.max(1,.8/zoom));
...
function render(){for(const id of multi)if(!get(id))multi.delete(id);
 ...
 if($('#compact-title')){const ans=mainConclusion();$('#compact-title').textContent=(board.title==='What am I trying to establish?'?'Untitled question':board.title)+(ans?' → '+wording(ans):'')}
 ...
 renderGroups();measureBounds();layoutCanvas();
 renderEdges();renderInspector();if(walking)renderWalk();
 for(const hook of renderHooks)hook();
 syncUrl();
}
// Groups: named sets of cards, drawn as a tinted outline behind them. A card
// can sit in several groups, so overlapping arguments show as overlapping
// outlines. A group can be collapsed into one card that stands in for its
// members, connections included, so the board reads at a higher level.
const GROUP_COLORS=['#3f8a63','#4a78b0','#8766b5','#b58324','#b5566a','#6f7582'];
const groupsOf=id=>(board.groups||[]).filter(g=>g.members.includes(id));
// The collapsed group a card disappears into (the biggest, if several).
function collapsedOf(id){let best=null;for(const g of board.groups||[])if(g.collapsed&&g.members.includes(id)&&(!best||g.members.length>best.members.length))best=g;return best}
...
function groupHead(g){const c=GROUP_COLORS[g.color%GROUP_COLORS.length],edit=author&&!readerMode;
 return `<div class="group-tab" data-group-drag="${esc(g.id)}" style="--g:${c}"><button class="group-fold" data-group-fold="${esc(g.id)}" aria-label="${g.collapsed?'Expand':'Collapse'} group" title="${g.collapsed?'Expand':'Collapse into one card'}">${g.collapsed?'▸':'▾'}</button><span class="group-label" data-group-rename="${esc(g.id)}" title="${edit?'Double-click to rename · drag to move the group':''}">${esc(g.label||'Untitled group')}</span>${edit?`<button class="group-color" data-group-color="${esc(g.id)}" aria-label="Change colour" title="Change colour"></button><button class="group-remove" data-group-remove="${esc(g.id)}" aria-label="Ungroup" title="Ungroup (keeps the cards)">✕</button>`:''}</div>`}
function renderGroups(){const canvas=$('#canvas');if(!canvas)return;canvas.querySelectorAll('.group,.group-card').forEach(e=>e.remove());
 board.groups=(board.groups||[]).filter(g=>(g.members=g.members.filter(id=>get(id))).length);if(tab!=='map'||walking)return;
 const visible=new Set(visibleNodes().map(n=>n.id)),first=canvas.firstChild,frag=document.createDocumentFragment(),cards=[];
 // Collapsed groups first: their cards are what other groups and connections attach to.
 for(const g of board.groups){if(!g.collapsed||collapsedOf(g.members[0])!==g&&g.members.every(id=>collapsedOf(id)!==g))continue;
  const ps=g.members.map(get).filter(active).map(pos);if(!ps.length)continue;const x=Math.min(...ps.map(p=>p.x)),y=Math.min(...ps.map(p=>p.y));
  const lead=g.members.map(get).find(n=>n.type==='conclusion')||g.members.map(get).find(n=>n.type==='claim')||get(g.members[0]),count=g.members.length;
  const el=document.createElement('div');el.className='group-card';el.dataset.groupCard=g.id;el.style.cssText=`left:${x}px;top:${y}px;--g:${GROUP_COLORS[g.color%GROUP_COLORS.length]}`;
  el.innerHTML=`${groupHead(g)}<p class="group-card-count">${count} card${count>1?'s':''} inside</p>${lead?`<p class="group-card-lead">${esc(wording(lead)||lead.quote||'')}</p>`:''}<button class="group-open" data-group-fold="${esc(g.id)}">▸ Show the ${count} cards</button>`;
  canvas.append(el);cards.push(g)}
 // Nested groups get more room, so an outer outline always clears the inner ones.
 const rects=[];for(const g of board.groups){if(cards.includes(g)||g.collapsed)continue;
  const ends=[...new Map(g.members.map(id=>endpoint(id,visible)).filter(Boolean).map(e=>[e.key,e])).values()];if(!ends.length||ends.every(e=>e.group&&e.group!==g&&ends.length===1))continue;
  rects.push({g,ends,box:{x:Math.min(...ends.map(e=>e.el.offsetLeft)),y:Math.min(...ends.map(e=>e.el.offsetTop)),r:Math.max(...ends.map(e=>e.el.offsetLeft+e.el.offsetWidth)),b:Math.max(...ends.map(e=>e.el.offsetTop+e.el.offsetHeight))}})}
 for(const r of rects){const inner=rects.filter(o=>o!==r&&o.g.members.length<r.g.members.length&&o.g.members.every(id=>r.g.members.includes(id))).length,pad=16+30*inner;
  const el=document.createElement('div');el.className='group';el.dataset.group=r.g.id;el.style.cssText=`left:${r.box.x-pad}px;top:${r.box.y-pad}px;width:${r.box.r-r.box.x+2*pad}px;height:${r.box.b-r.box.y+2*pad}px;--g:${GROUP_COLORS[r.g.color%GROUP_COLORS.length]}`;
  el.innerHTML=groupHead(r.g);frag.append(el)}
 canvas.insertBefore(frag,first)}
function groupCards(ids){ids=[...new Set(ids)].filter(id=>get(id));if(!ids.length)return;board.groups=board.groups||[];const g={id:uid(),label:'',members:ids,color:board.groups.length%GROUP_COLORS.length};board.groups.push(g);persist();render();renameGroup(g.id)}
function renameGroup(id){const g=(board.groups||[]).find(x=>x.id===id),span=$(`#canvas [data-group-rename="${CSS.escape(id)}"]`);if(!g||!span||readerMode)return;
 const input=document.createElement('input');input.className='group-input';input.value=g.label;input.placeholder='Name this group';span.replaceWith(input);input.focus();input.select();
...
document.addEventListener('click',e=>{const t=e.target.closest('[data-group-fold],[data-group-color],[data-group-remove],[data-group-add],[data-group-leave],[data-group-multi]');if(!t||readerMode&&!t.dataset.groupFold)return;const d=t.dataset,find=id=>(board.groups||[]).find(g=>g.id===id);
 if(d.groupFold){const g=find(d.groupFold);if(g){g.collapsed=!g.collapsed;if(g.collapsed&&(g.members.includes(selected)||[...multi].some(id=>g.members.includes(id)))){selected=null;multi.clear()}api.requestLayout?.();persist();render()}}
 else if(d.groupColor){const g=find(d.groupColor);if(g){g.color=(g.color+1)%GROUP_COLORS.length;persist();render()}}
 else if(d.groupRemove){board.groups=board.groups.filter(g=>g.id!==d.groupRemove);persist();render();notify('Ungrouped. The cards stay where they are.',{label:'Undo',run:()=>undo()})}
 else if(d.groupMulti!==undefined)groupCards(multi.size?[...multi]:selected?[selected]:[]);
 else if(d.groupLeave){const[gid,id]=d.groupLeave.split('|'),g=find(gid);if(g){g.members=g.members.filter(m=>m!==id);persist();render()}}});
document.addEventListener('change',e=>{const s=e.target.closest('[data-group-add]');if(!s||!s.value)return;const g=(board.groups||[]).find(x=>x.id===s.value),ids=multi.size?[...multi]:[selected];if(g){g.members=[...new Set([...g.members,...ids])];persist();render();notify(`Added to “${g.label||'Untitled group'}”.`)}});
document.addEventListener('dblclick',e=>{const t=e.target.closest('[data-group-rename]');if(t&&author&&!readerMode){e.preventDefault();renameGroup(t.dataset.groupRename);return}const c=e.target.closest('.group-card');if(c&&!e.target.closest('button,input')){const g=(board.groups||[]).find(x=>x.id===c.dataset.groupCard);if(g){g.collapsed=false;api.requestLayout?.();persist();render()}}});
// Drag a group by its label (or a collapsed group by its card) to move all its cards; a click selects them.
$('#viewport').addEventListener('pointerdown',e=>{const h=e.target.closest('[data-group-drag],.group-card');if(!h||e.button!==0||e.target.closest('button,input,a')||spaceDown)return;
 const g=(board.groups||[]).find(x=>x.id===(h.dataset.groupDrag||h.dataset.groupCard));if(!g)return;e.stopImmediatePropagation();e.preventDefault();
 const sx=e.clientX,sy=e.clientY,origin=g.members.map(get).filter(Boolean).map(n=>({n,x:n.x,y:n.y}));let moved=false;
 ...
 const up=()=>{removeEventListener('pointermove',move);removeEventListener('pointerup',up);
  if(moved){origin.forEach(o=>delete o.n.auto);persist();render()}
  else if(!g.collapsed){const ids=g.members.filter(id=>$(`#canvas [data-node="${CSS.escape(id)}"]`));multi=new Set(ids);selected=ids.at(-1)||null;if(multi.size<2)multi.clear();render()}};
...
$('#kind').onchange=e=>{board.kind=e.target.value;selected=null;persist();render()};$('#profit').onchange=e=>{board.profit=e.target.value;selected=null;persist();render()};$('#all-routes').onchange=e=>{allRoutes=e.target.checked;render()};$('#view').onclick=()=>{author=false;render()};$('#author').onclick=()=>{author=true;render()};$('#map-tab').onclick=()=>{tab='map';render()};if($('#outline-tab'))$('#outline-tab').onclick=()=>{tab='outline';render()};$('#sources-tab').onclick=()=>{tab='sources';render()};$('#zoom-in').onclick=()=>zoomTo((zoomTarget??zoom)*1.2);$('#zoom-out').onclick=()=>zoomTo((zoomTarget??zoom)/1.2);$('#fit').onclick=()=>{cancelAnimationFrame(zoomFrame);zoomFrame=0;zoomTarget=null;zoom=clampZoom(Math.min(1,(vpSize.w-80)/bounds.w,(vpSize.h-80)/bounds.h));render();scrollToCanvas(bounds.x+bounds.w/2-vpSize.w/2/zoom,bounds.y+bounds.h/2-vpSize.h/2/zoom);syncUrl()};
...
document.addEventListener('click',e=>{const el=e.target.closest('[data-select],[data-deselect],[data-walk-step],#prev-step,#next-step,[data-node]');if(!el||e.target.closest('a,[data-edit],[data-inspect-card],#edit-heading,[data-connect-handle],[data-delete],[data-evidence-toggle]'))return;if(el.hasAttribute('data-select'))activateCard(el.dataset.select,true);else if(el.hasAttribute('data-deselect')){selected=null;multi.clear();render()}else if(el.hasAttribute('data-walk-step')){step=Number(el.dataset.walkStep);renderWalk()}else if(el.id==='prev-step'){step--;renderWalk()}else if(el.id==='next-step'){if(step===sequence().length-1){walking=false;render()}else{step++;renderWalk()}}else if(el.dataset.node&&!dragMoved){if((e.shiftKey||e.metaKey||e.ctrlKey)&&author&&!readerMode)toggleMulti(el.dataset.node);else{multi.clear();activateCard(el.dataset.node)}}});
...
document.addEventListener('keydown',e=>{if(readerMode||walking||tab!=='map'||$('dialog[open]')||e.target.matches('input,textarea,select,[contenteditable]'))return;
 if(e.key==='Escape'&&(multi.size||selected)){multi.clear();selected=null;render()}
 else if(author&&(e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='g'&&(multi.size||selected)){e.preventDefault();groupCards(multi.size?[...multi]:[selected])}
 else if(author&&(e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==='a'){e.preventDefault();multi=new Set(visibleNodes().map(n=>n.id));selected=[...multi].at(-1)||null;if(multi.size<2)multi.clear();render()}
...
function validateBoard(input){
 ...
 if(input.groups!==undefined&&(!Array.isArray(input.groups)||input.groups.length>200))bad('Invalid groups.');const groups=(input.groups||[]).map(g=>{if(!g||!str(g.id,100)||!str(g.label||'',300)||!Array.isArray(g.members))bad('Invalid group.');return {id:g.id,label:g.label||'',members:[...new Set(g.members.filter(id=>ids.has(id)))],color:Number.isInteger(g.color)&&g.color>=0?g.color%6:0,...(g.collapsed===true?{collapsed:true}:{})}}).filter(g=>g.members.length);
...
const api={get board(){return board},get session(){return session},workspace,get,uid,esc,notify,render,persist,freePosition,select,validateBoard,
 ...
 setAuthor(v){author=!!v;render()},get inboxOpen(){return inboxOpen},setInbox(v){inboxOpen=!!v;render()},get zoom(){return zoom},canvasPoint(cx,cy){const r=$('#canvas').getBoundingClientRect();return {x:Math.round((cx-r.left)/zoom),y:Math.round((cy-r.top)/zoom)}},get tab(){return tab},isHidden:id=>{const n=get(id);return !!n&&evidenceHidden(n)},showEvidenceFor,wordLabel,allowedWords,fitWord,linkRole,supportersOf,WORD_CHOICES,mainConclusion,isInterim,setTab(t){tab=t;render()},openCard,onRender(f){renderHooks.push(f)},commit(record=true){commitBoard(record)},typeLabel,startWalk(){walking=true;step=0;tab='map';render()},currentStep:()=>walking?sequence()[step]?.id:null,sequence:()=>sequence().map(n=>n.id)};
