...
2. **Open the side panel** (*Side panel* in the popup, or Alt+R) — the live "you are here" view. Your current page is ringed in green, pages with open tabs are lit, everything else is parked (dimmed). Click a node to switch to its tab or reopen it; **⏏ Park other tabs** closes everything except where you are, safely.
   The panel's header shows the workspace you're recording into (click to switch), pause, and a ⋯ menu (full map, Ask this workspace, Park other tabs, Glossary, Translation settings). Under the map, a dock has two tabs: **This page** (Star, *Save selection* — highlight text first, same as right-click → Save passage — Screenshot, a jot box, translate, and Ask this page) and **Board** (pick a board, open it, walk through its reading order).
...
- **Hide translations** — saved translations re-apply automatically on revisit, which can trip up script-heavy pages. *Show German* in the popup or the side panel puts the page back to German and stops re-applying them there, while keeping the hover toolbar; the cache is kept, so *Show translations* is instant. If the page's own scripts are still confused, *Reload page* gives them a clean start — the setting sticks. *Amtshelfer: off* switches the whole module off for the page instead.
...
- **On the board**: click selects; ✎ Edit or double-click opens the editor; the trash button, *Delete card* in the editor, or the Delete key removes cards, with *Undo* right in the confirmation. In the evidence editor the source link is clickable, with ✎ Edit to change it. Drag a card's → handle onto another card to connect them. Drag the divider beside the side panel to resize it (double-click collapses it).
- **Groups**: select cards (Shift-click or drag a box) and press ⌘G, or use *New group* in the side panel, then name the group. Groups nest to any depth: groups a selection fully covers go inside the new group, so grouping groups builds a board within the board. Drag a group's label onto another group to put it inside (the target lights up and says so); ⇱ takes it back out. Each group has one home, but a card can be in several groups, so overlapping arguments show as overlapping outlines. Drag cards onto a group to add them. Drag a label to move everything in the group; click it to select its cards (Shift-click adds another group's).
- **Questions at every level**: a group's name is its question and its face is its answer, so a board inside the board is just the level you're focused on. Focus on a group and the heading becomes that group's question and answer (with *Part of: …* above it), the breadcrumbs become a trail of questions, the Outline shows just that group's lines (*↑ Step out* returns), Walk through covers just its steps, and anything you add joins it. The board's own question is always the furthest-out level: *Board ▾ › Wrap in a bigger question* turns everything so far into a group under a new base question, and a group that holds every card offers *Make it the base question*, which turns the old base question into a group around its answer and everything that leads to it.
- **Folding makes room**: where cards are saved always describes the board with every group open. Collapsing a top-level group gives back the space it no longer needs: what sits to its right (in its rows) slides left, what sits below it (in its columns) slides up. Opening it reverses that exactly and nudges any card that has wandered into its outline back out. Anything that would land on something else stays put, and moves slide briefly so you can see them. Groups inside groups just shrink their parent's outline.
- **Tidy groups** (*Board ▾*): moves cards that sit inside a group's outline without belonging to it just outside it, the shortest way (undo brings them back).
- **Reading at any level**: ▾ collapses a group into its *face*, the card it argues for: by default the conclusion (or claim) inside it that nothing else inside leads to; ☆ next to a group in the side panel picks a different one. The collapsed card sits where the face was, and connections to anything inside attach to it. ⤢ focuses on a group: everything else fades, the view fits it, and breadcrumbs (*Whole board › Outer › This group*) replace the legend; Esc steps out one level. Zoomed far out (down to 15%), cards reduce to their headline, which keeps a readable minimum size (about 11px on screen, up to four lines), and group labels grow to match.
- **Layout**: navigation (trail, argument picker, help, evidence inbox, and a *Board ▾* menu for new/open/save/export and editing the base question) sits in one row, and the board's tools, View/Author and Walk through in another; on a narrow window the tools shrink to icons with tooltips. Drag the divider under the heading (or double-click it) to fold the heading away.
- **Side panel**: opens by itself when you select a card, cards or a group, and steps aside when you clear the selection (Esc). ◧ keeps it open, with the walkthrough steps and board summary.
- **Selecting groups**: click a group's label or a collapsed group's card to select the group (its details open in the side panel); Shift-click adds more groups or cards, and a box drag picks up collapsed groups too. ⌘G then groups everything selected, with the selected groups going inside the new one.
- **Double-click to add**: double-click empty board space and pick what to add (Thought, Fact, Claim, Conclusion, Open question, Evidence; or press its letter). The card lands where you clicked; inside a group's outline, or while focused on a group, it joins that group.
- **Open canvas**: the spatial board has no scrollbars. Pan with Space+drag or two-finger scroll; you can always go half a screen past the outermost card in any direction, so any group of cards can sit in the middle of the view. ⌘-scroll (Ctrl-scroll elsewhere) or a trackpad pinch zooms smoothly around the pointer; *Fit* frames every card.
- **Evidence folds away**: on the spatial board, evidence tucks under the card it backs up, so the board reads as the line of reasoning. *▸ Show N sources* on a card opens just its evidence; *Show all evidence* in the footer opens everything. Evidence you just attached, or pick from Sources, opens where it lives; evidence attached to nothing stays visible. The outline and walkthrough always show everything.
- **Evidence inbox panel**: drag a captured passage or jotted thought onto a card or outline line to back it up, or onto empty board space to place it. *Archive* hides something you saved by accident (restore or delete it from *Archived*); cards already on a board keep their own copy either way.
- **From your trail**: the inbox's second tab searches the pages you already visited in this workspace (titles, addresses and the page text saved when you visited). Drag a page onto the board as a source card, or *Pick a passage* to read its saved text and save the exact words without going back to the page. Passages picked this way are marked as coming from saved text, since the live page may have changed.
...
- **Collect**: select text → right-click → *Save passage as evidence*, or *Save screenshot as evidence*. You stay on the page; a small confirmation shows where it went (the workspace of the board you last had open), with *Attach…* (pick the line on your board it backs up, or objects to, without opening the board), *Open board* and, if you were browsing in another workspace, *Move to …*. Saving from a paragraph Amtshelfer translated saves the original German paragraph as the quote (so the passage link lands on the real source) and keeps your English selection as its translation. For screenshots you drag a box around what to save (a click takes the whole visible page, Esc cancels); the card keeps the crop and the full view as context. The side panel offers the same attach step after capturing from it. Captures land in the board's **Evidence inbox**.
