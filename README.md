...
- **Collect**: select text → right-click → *Save passage as evidence*, or *Save screenshot as evidence*. You stay on the page; a small confirmation shows where it went (the workspace of the board you last had open), with *Attach…* (pick the line on your board it backs up, or objects to, without opening the board), *Open board* and, if you were browsing in another workspace, *Move to …*. For screenshots you drag a box around what to save (a click takes the whole visible page, Esc cancels); the card keeps the crop and the full view as context. The side panel offers the same attach step after capturing from it. Captures land in the board's **Evidence inbox**.
