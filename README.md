...
- **Groups**: select cards (Shift-click or drag a box) and press ⌘G, or use *New group* in the side panel, then name the group. A card can be in several groups, so overlapping arguments show as overlapping outlines, and a group inside another gets a wider outline. Drag a card (or several) onto a group, or onto a collapsed group's card, to add it: the group lights up and says what dropping will do. Drag a group's label to move all its cards; click the label to select them. ▾ collapses a group into one card that stands in for everything inside it, connections included, so you can read the board at a higher level; double-click it or *Show the cards* to open it again. Labels stay readable when zoomed out.
