...
- **Two ways to read the outline**: *Answer first* ("Y *because* X", your conclusion on top) or *Build up* ("*since* X … *and so* Y", reasons first). Same argument, same data; in Build up, start a line with `so` to draw a conclusion from the lines above it. Conclusions can nest: interim conclusions become reasons for the one that answers the question, which is shown at the top of the page. Connections the nesting can't show are drawn as arrows in the margin.
- **One vocabulary everywhere**: the Answer first / Build up switch applies to the outline and the board alike. Answer first puts the point on the left of the board with *because* arrows to its reasons; Build up puts reasons on the left with *so* arrows to the point. Disagreement is always an *objection* ("*one objection:* …", *objects to*). Starting a line with `but` makes it and the line above two reasons for a new line you then write, since "X, but Y" usually means "there's also Y to reckon with", not "Y proves X wrong". Which connection fits is decided by the card it starts from (a quote is a source, a question raises a question, anything else is a reason), in every view.
