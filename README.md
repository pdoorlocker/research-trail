...
- **One vocabulary everywhere**: the board's arrows read *so* / *yet*, a card's details read "*because* …", and the walkthrough introduces steps that follow from earlier ones with *and so*. Which connection fits is decided by the card it starts from (a quote is a source, a question raises a question, anything else is a reason), in every view.
